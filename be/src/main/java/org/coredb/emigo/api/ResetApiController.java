...
public class ResetApiController implements ResetApi {
    ...
    public ResponseEntity<Contact> resetCheck(@NotNull @ApiParam(value = "reset token", required = true) @Valid @RequestParam(value = "token", required = true) String token
) {
    try {
      Contact contact = profileService.getReset(token);
    ...
    public ResponseEntity<Void> resetPassword(@NotNull @ApiParam(value = "confirmation token", required = true) @Valid @RequestParam(value = "token", required = true) String token
,@NotNull @ApiParam(value = "updated account password", required = true) @Valid @RequestParam(value = "password", required = true) String password
      ...
      try {
        profileService.setReset(token, password);
